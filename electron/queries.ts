...
export function getCategories() {
  ...
  const grouped: Record<
    string,
    { major: string; minors: { id: number; name: string; preset: boolean }[] }[]
  > = {
    expense: [],
    income: [],
  };
...
export function addCategory(cat: { type: string; major: string; minor: string }): {
  ok: boolean;
  error?: string;
} {
  const db = getDb();
...
export function updateCategory(cat: { id: number; major: string; minor: string }): {
  ok: boolean;
  error?: string;
} {
  const db = getDb();
