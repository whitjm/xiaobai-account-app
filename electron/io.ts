...
// —— 导出 Excel ——
export async function exportExcel(): Promise<{
  ok: boolean;
  canceled?: boolean;
  count?: number;
  filePath?: string;
...
export async function restoreData(): Promise<{
  ok: boolean;
  canceled?: boolean;
  count?: number;
  error?: string;
  ...
  for (const r of data.records as {
    type: string;
    amount: number;
    major: string;
    minor: string;
...
export async function importExcel(): Promise<{
  ...
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    const [typeCn, amount, major, minor, date, note] = row as [
      string,
      number,
      string,
      string,
