...
**内容:**
- 配置 NSIS 安装包选项（允许自定义目录、桌面/开始菜单快捷方式）
- 配置中文安装界面（`language: 2052`）
- 代码签名（未申请，暂无）

**验收标准:**
- [x] 安装包用户体验良好（中文 NSIS 安装界面）
- [x] 桌面快捷方式创建正常（`createDesktopShortcut: true`）
- [x] 卸载功能正常（NSIS 内置卸载程序）
...
**阶段6验收清单:**
- [x] Windows 安装包生成（本地验证通过，112MB）
- [x] macOS 安装包生成（配置文件就绪，无 macOS 环境实测）
- [x] Release 自动化正常（打 Tag 自动创建 Release）
- [x] 安装包可发布（中文 NSIS 安装界面，快捷方式正常）
